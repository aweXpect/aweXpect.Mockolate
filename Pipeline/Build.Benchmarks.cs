...
partial class Build
{
	Target BenchmarkDotNet => _ => _
		.Executes(() =>
		{
			...
			DotNetBuild(s => s
				.SetProjectFile(Solution.Benchmarks.aweXpect_Mockolate_Benchmarks)
				.SetConfiguration(Configuration.Release)
				.EnableNoLogo());

			DotNet(
				$"{Solution.Benchmarks.aweXpect_Mockolate_Benchmarks.Name}.dll --exporters json --filter * --artifacts \"{benchmarkDirectory}\"",
				Solution.Benchmarks.aweXpect_Mockolate_Benchmarks.Directory / "bin" / "Release");
		});
	...
	Target BenchmarkResult => _ => _
		...
		.Executes(async () =>
		{
			string fileContent = await File.ReadAllTextAsync(ArtifactsDirectory / "Benchmarks" / "results" /
			                                                 "aweXpect.Mockolate.Benchmarks.HappyCaseBenchmarks-report-github.md");
		...
		.Executes(async () =>
		{
			...
			if (int.TryParse(prNumber, out int prId))
			{
				...
				else
				{
					...
					await gitHubClient.Issue.Comment.Update("Testably", "aweXpect.Mockolate", commentId.Value, body);
				}
			}
		});
