...
public sealed partial class ThatVerificationResultIs
{
	public sealed class ThenTests
	{
		...
		public async Task Then_ShouldVerifyInOrder()
		{
			...
			await That(sut.Verify.Invoked.MyMethod(3)).Then(m => m.Verify.Invoked.MyMethod(4));
			await That(async Task () => await That(sut.Verify.Invoked.MyMethod(2)).Then(m => m.Verify.Invoked.MyMethod(1)))
				...
				.WithMessage("""
					...
					""");
			await That(sut.Verify.Invoked.MyMethod(1)).Then(m => m.Verify.Invoked.MyMethod(2), m => m.Verify.Invoked.MyMethod(3));
		}
		...
		public async Task Then_WhenNoMatch_ShouldReturnFalse()
		{
			...
			await That(async Task () => await That(sut.Verify.Invoked.MyMethod(6)).Then(m => m.Verify.Invoked.MyMethod(4)))
				.Throws<XunitException>()
				.WithMessage("""
					Expected that the Mock<ThatVerificationResultIs.IMyService>
			...
			await That(async Task () => await That(sut.Verify.Invoked.MyMethod(1)).Then(m => m.Verify.Invoked.MyMethod(6), m => m.Verify.Invoked.MyMethod(3)))
				.Throws<XunitException>()
				.WithMessage("""
					Expected that the Mock<ThatVerificationResultIs.IMyService>
			...
			await That(async Task () => await That(sut.Verify.Invoked.MyMethod(1)).Then(m => m.Verify.Invoked.MyMethod(2), m => m.Verify.Invoked.MyMethod(6)))
				.Throws<XunitException>()
				.WithMessage("""
					Expected that the Mock<ThatVerificationResultIs.IMyService>
					invoked method MyMethod(1), then
